    ...
    public String searchSimilar(@RequestParam(required = false) Long baseMotorcycleId,
        ...
        if (minEngineSize != null || maxEngineSize != null) {
            similarMotorcycles = similarMotorcycles.stream()
                    .filter(m -> {
                        if (m.getEngineSize() == null) return false;
                        double engineSize = m.getEngineSize().doubleValue();
                        boolean minOk = minEngineSize == null || engineSize >= minEngineSize;
                        boolean maxOk = maxEngineSize == null || engineSize <= maxEngineSize;
                        return minOk && maxOk && !m.getId().equals(baseMotorcycleId);
                    })
                    .collect(java.util.stream.Collectors.toList());
        } else {
            // Excluir la moto base si no hay filtros
            similarMotorcycles = similarMotorcycles.stream()
                    .filter(m -> !m.getId().equals(baseMotorcycleId))
                    .collect(java.util.stream.Collectors.toList());
        }
